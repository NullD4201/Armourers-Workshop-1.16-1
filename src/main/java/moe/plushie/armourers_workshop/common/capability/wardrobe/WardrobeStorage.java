...
public class WardrobeStorage implements IStorage<IWardrobeCap> {
    ...
    @Override
    public NBTBase writeNBT(Capability<IWardrobeCap> capability, IWardrobeCap instance, Direction side) {
        ...
        CompoundNBT dyeCompund = new CompoundNBT();
    ...
    @Override
    public void readNBT(Capability<IWardrobeCap> capability, IWardrobeCap instance, Direction side, NBTBase nbt) {
        ...
        if (compound.hasKey(TAG_DYE, NBT.TAG_COMPOUND)) {
            CompoundNBT dyeCompund = compound.getCompoundTag(TAG_DYE);
